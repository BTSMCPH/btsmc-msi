<x-app-layout>
    ...
    <div class="p-4 bg-white rounded-lg shadow-xs">
        ...
        @if (session('success'))
            <div id="success-alert" x-data="{ show: true }" x-show="show" class="p-4 mb-4 text-sm text-green-700 bg-green-100 rounded-lg shadow">
        ...
        <table id="role-table" class="w-full border border-collapse border-gray-300 table-auto stripe hover dataTable dtr-inline" style="width:100%;">
            <thead>
                <tr>
                    <th class="w-1/3 px-4 py-2 text-left">{{ __('Name') }}</th>
                    <th class="w-1/3 px-4 py-2 text-left">{{ __('Guard Name') }}</th>
                    <th class="w-1/3 px-4 py-2 text-left">{{ __('Permissions') }}</th>
